      ...
      <div className={`flex h-full flex-col p-4 ${collapsed ? "pt-16 md:pt-20" : "pt-4 md:pt-6"}`}>
        ...
        <nav className="mt-6 grid gap-2">
          {navItems.map((item) => {
            ...
            return (
              <SidebarTooltip key={item.value} show={collapsed} content={tooltipLabel} className="w-full">
                <Button
                  variant="ghost"
                  className={
                    isActive
                      ? `relative h-11 bg-amber-50/25 text-amber-50 hover:bg-amber-50/30 ${
                          collapsed ? "mx-auto h-12 w-12 justify-center p-0 overflow-visible" : "justify-start gap-2"
                        }`
          ...
          {me ? (
            collapsed ? (
              <div className="flex justify-center">
                <SidebarTooltip show={collapsed} content={`${displayName} (${me.role})`} className="inline-flex">
                  <div
